  ...
  switch (action.type) {
    ...
    case Types.TOGGLE_MODE:
      ...
      if (
        ...
        currentModeIndex = 0;
        if (state.displayModes[currentModeIndex] == "list") {
          currentModeIndex++;
        }
      } else {
        currentModeIndex++;
    ...
    case Types.TOGGLE_MICROPHONE: {
      ...
      if (!state.disableSounds) {
        const audio = new Audio(
          !currentStatus ? sounds.mute_off : sounds.mute_on
        );
        audio.play().catch((e) => {
          console.error("Could not play the sound", e.message);
        });
    ...
    case Types.TOGGLE_REQUESTED_VIDEO: {
      ...
      let rv = !requested
        ? [...state.requestedVideos, participant_id]
        : state.requestedVideos.filter((id) => id != participant_id);
      return {
        ...state,
        requestedVideos: rv,
      };
    ...
    case Types.SET_REQUESTED_VIDEO: {
      ...
      const requested = state.requestedVideos.indexOf(participant_id) > -1;
      if (fw_state === requested) return state;
      let rv = !requested
        ? [...state.requestedVideos, participant_id]
        : state.requestedVideos.filter((id) => id != participant_id);
      return {
