        ...
        return (dispatch, getState) => {
            ...
            if (!interval) {
                interval = setInterval(() => {
                    const { voxeet: { participants, activeSpeaker } } = getState()
                    ...
                    if (!activeSpeaker.forceActiveUserEnabled) {
                        for (let participant of participants.participants) {
                            if (participant.participant_id)
                                Sdk.instance.isUserSpeaking(participant.participant_id , (status) => {
                                    participant.isSpeaking = status
                                })
                        }
                        const participantsConnected = participants.participants.filter((p) => p.isConnected)
                        const participant = participantsConnected.length === 1 ? participantsConnected[0] : (participants.participants.find((p) => p.isSpeaking) || activeSpeaker.activeSpeaker)
