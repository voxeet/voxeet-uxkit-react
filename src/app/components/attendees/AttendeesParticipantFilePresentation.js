...
class AttendeesParticipantFilePresentation extends Component {
    ...
    updateFilePresentation(position) {
        ...
        this.props.dispatch(ConferenceActions.updateFilePresentation(filePresentationId, position))
    }

    nextFilePresentation() {
        const { imageCount, filePresentationPosition, filePresentationId } = this.props.filePresentationStore
        if (filePresentationPosition < (imageCount - 1)) {
            this.props.dispatch(ConferenceActions.updateFilePresentation(filePresentationId, filePresentationPosition + 1))
    ...
    handleMouseMove(e) {
        ...
        clearTimeout(this.state.timeout);
        if (!this.state.mouseIn) this.state.timeout = setTimeout(() => this.setState({mouseMoving:false}), 2000);
    }
    ...
    render() {
        ...
        const { fileUrl, imageCount, filePresentationPosition, thumbnails } = this.props.filePresentationStore
        ...
        return (
            ...
            >
                <img className="file-presentation-img" src={fileUrl} />
                { isFilePresentation && thumbnails.length > 1 && 
                    <Fragment>
                        <div className="thumbnails" id="thumbnails">
                            { 
                                thumbnails.map((thumb, i) => {
                                    return(
                                        <a key={i} id={ "thumbnail-" + i } onClick={() => { this.updateFilePresentation(i) } }>
                                            <div className={i == filePresentationPosition ? "thumbnail-container current" : "thumbnail-container"}>
                        ...
                        <div onMouseEnter={this.handleMouseEnter} onMouseLeave={this.handleMouseLeave} className={mouseMoving ? "controls-file-presentation" : "controls-file-presentation hidden-controls"}>
                            <button className={filePresentationPosition == 0 ? "prev disabled" : "prev"} onClick={this.prevFilePresentation}>{strings.prev}</button>
                            <button className={filePresentationPosition == (imageCount - 1) ? "next disabled" : "next"} onClick={this.nextFilePresentation}>{strings.next}</button>
                        </div>
                    </Fragment>
                }
            </div>
