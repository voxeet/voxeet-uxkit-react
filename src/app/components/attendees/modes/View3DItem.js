...
class View3DItem extends Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
      ...
      if (((nextProps.size != this.state.size) && !nextProps.participant.isMoved) || (this.state.posX == -1 && this.state.posY == -1)) {
          ...
          setUserPosition(participant.participant_id, relativePosition, position, false)
          this.setState({ parent: parent, participant: participant, posX: position.posX, posY: position.posY, size: size })
          return true
        } else if (nextState.posX != this.state.posX || nextState.posY != this.state.posY) {
          this.setState({ parent: parent, posX: nextProps.participant.x, posY: nextProps.participant.y, size: size, participant: participant })
          return true
        }
        return true
    }
    ...
    render() {
        const { participant, toggleMicrophone, kickParticipant, isAdmin, isAdminActived } = this.props
            ...
            <Draggable
                ...
                <div>
                    <div className='participant-bubble'>
                        {participant.stream ?
                            <AttendeesParticipantVuMeter participant={participant} isVideo={true} />
                            :
                            <AttendeesParticipantVuMeter participant={participant} />
                        }
                    </div>
                    <AttendeesParticipantBar kickParticipant={kickParticipant} isAdminActived={isAdminActived} isAdmin={isAdmin} participant={participant} toggleMicrophone={toggleMicrophone} is3D={true} />
                </div>
            </Draggable>
        )
    }
    ...
    handleDrag(event, ui) {
        ...
        if (this.state.timeout == true) {
          ...
          setTimeout(() => {
              setUserPosition(participant.participant_id, relativePosition, newPosition, true)
    ...
    _canMove(ui) {
        ...
        return (ui.x < (parent.clientWidth - 140) && ui.x > 10 && ui.y < (parent.clientHeight - 35 - 120) && ui.y > 0)
    }
}
