...
class TileVideo extends Component {
    ...
    render() {
        const { participant, toggleMicrophone, isWidgetFullScreenOn, isAdmin, kickParticipant, isAdminActived, mySelf } = this.props
        const photoUrl = participant.avatarUrl || userPlaceholder
        return (
            <span className="tile-video video-frame">
                {isWidgetFullScreenOn &&
                    <AttendeesParticipantBar toggleAutomatically={true} kickParticipant={kickParticipant} isAdminActived={isAdminActived} isAdmin={isAdmin} participant={participant} toggleMicrophone={toggleMicrophone} />
                }
                {participant.stream ?
