  ...
  render() {
    ...
    return (
      <div
        ...
        className={
          show ? "onactivespeakeroverlay" : "onactivespeakeroverlay-hidden"
        }
      >
        {activeSpeakerChecker &&
        activeSpeakerChecker.stream &&
        activeSpeakerChecker.stream.active &&
        activeSpeakerChecker.stream.getVideoTracks().length > 0 ? (
          <img src={cameraOn} />
        ) : (
          <img src={cameraOff} />
        )}
        <p>
          {activeSpeakerChecker && `${activeSpeakerChecker.name} is speaking`}
        </p>
      </div>
    );
  }
