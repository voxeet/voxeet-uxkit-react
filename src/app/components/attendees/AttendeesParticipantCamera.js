...
class AttendeesParticipantCamera extends Component {
  ...
  render() {
    ...
    let active =
      participantIds && participantIds.indexOf(participant.participant_id) > -1;
    let pinned =
      requestedVideos &&
      requestedVideos.indexOf(participant.participant_id) > -1;
    return (
      <span className="participant-camera">
        <a
          className={
            "icon-camera-participant-" +
            (active ? "on" : "off") +
            (pinned ? "pinned" : "")
          }
          onClick={(e) => {
        ...
        >
          <img
            ...
            title={
              pinned
                ? strings.pinnedCameraOn
                : active
                ? strings.pinnedCameraOff
                : strings.pinnedCameraOff
            }
          />
        </a>
