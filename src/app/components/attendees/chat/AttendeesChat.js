...
class AttendeesChat extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.attendeesChatOpened) {
      var scrollBar = document.getElementById("chat-scrollbar");
      if (scrollBar && prevProps.chatStore != this.props.chatStore)
    ...
    if (content.trim().length > 0) {
      ...
      this.props.dispatch(
        ChatActions.addMessage({
          ...payload,
          content: !chatOptions.autoLinker
            ? content
            : Autolinker.link(content.trim()),
        })
      );
      this.props.dispatch(
  ...
  render() {
    const { participants, currentUser, attendeesChatOpened, chatOptions } =
      this.props;
