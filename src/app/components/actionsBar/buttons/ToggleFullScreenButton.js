...
class ToggleFullScreenButton extends Component {
    ...
    render() {
        ...
        return (
            <li>
                <a data-tip data-for="toggle-fullscreen"
                    onClick={() => toggleFullScreen()}
                    className="icn-toggle-sidebar"
                    title={isWidgetFullScreenOn ? strings.minimize : strings.fullscreen}>
                    <span className={isWidgetFullScreenOn ? 'icon-fullscreen-off' : 'icon-fullscreen-on'}></span>
                </a>
                <ReactTooltip id="toggle-fullscreen" place={tooltipPlace} effect="solid" className="tooltip">{isWidgetFullScreenOn ? strings.minimize : strings.fullscreen}</ReactTooltip>
            </li>
        )
    }
}
