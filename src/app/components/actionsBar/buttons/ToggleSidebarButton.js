...
class ToggleSidebarButton extends Component {
    ...
    render() {
        ...
        return (
            <li>
                <a data-tip data-for="toggle-sidebar"
                    onClick={() => toggleWidget()} className="icn-toggle-sidebar"
                    title={isWidgetOpened ? strings.close : strings.open}>
                    <span className="icon-arrow-back-large"></span>
                </a>
                <ReactTooltip id="toggle-sidebar" place={tooltipPlace} effect="solid" className="tooltip">{isWidgetOpened ? strings.close : strings.open}</ReactTooltip>
            </li >)
    }
}
