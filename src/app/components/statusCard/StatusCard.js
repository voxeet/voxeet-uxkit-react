...
class StatusCard extends Component {
    ...
    joinConference() {
        const { conferenceAlias, constraints, isJoined, liveRecordingEnabled, conferenceId, ttl, rtcpmode, mode, videoCodec, userInfo } = this.props
        if (!isJoined) {
            this.props.dispatch(ConferenceActions.subscribeConference(conferenceAlias)).then(() => {
                if (conferenceId != null) {
                  ...
                  this.props.dispatch(ConferenceActions.join(conferenceAlias, constraints, liveRecordingEnabled, ttl, rtcpmode, mode, videoCodec, userInfo))
                }
            })
        }
    }
            ...
            <div className="vxt-conference-card-status">
                ...
                {!isJoined &&
                    <div className="expand" onClick={this.joinConference}>
                        <i className='icon-phone'> <span>{strings.join}</span></i>
                    </div>
                }
                {isJoined && isModal &&
                    <div className="expand" onClick={this.toggleModalWidget}>
                        {modalOpened
                            ? <i className='icon-fullscreen-off'> <span>{strings.close}</span></i>
                            : <i className='icon-fullscreen-on'> <span>{strings.expand}</span></i>
                        }
                    </div>
                }
                {isJoined &&
                    <div className="leave" onClick={this.leaveConference}>
                        <i className='icon-phone-end-call'> <span>{strings.leavecall}</span></i>
                    </div>
                }
