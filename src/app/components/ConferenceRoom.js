...
class ConferenceRoom extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      isElectron: false,
      preConfig: (!props.isListener && !bowser.msie && !(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) && (!props.isWebinar || (props.isWebinar && props.isAdmin))) ? props.preConfig : false
    }
  ...
  startConferenceWithParams(preConfigPayload = null) {
    ...
    if (conferenceReplayId != null) {
      ...
      if (isDemo) {
        ...
        initialized.then(() => this.props.dispatch(ConferenceActions.joinWithConferenceId(conferenceId, constraintsUpdated, userInfo, videoRatio, isElectron, isListener, preConfigPayload)))
      } else if (autoJoin && conferenceReplayId == null) { // Autojoin when entering in fullscreen mode
        initialized.then(() => {
          this.props.dispatch(ConferenceActions.join(conferenceAlias, isAdmin, constraints, liveRecordingEnabled, ttl, rtcpmode, mode, videoCodec, userInfo, videoRatio, isElectron, isListener, preConfigPayload))
        })
      }
  ...
  render() {
    const { isListener, dispatch, options, isWidget, isModal, conferenceAlias, constraints, actionsButtons, attendeesList, attendeesChat, handleOnLeave, attendeesWaiting, isWebinar, isAdmin, logo } = this.props
    const { screenShareEnabled, filePresentationEnabled } = this.props.participantsStore
    const { preConfig } = this.state
    const { isJoined, conferenceId, initialized, isReplaying, conferenceReplayId, isElectron, isDemo, conferencePincode, hasLeft } = this.props.conferenceStore
    ...
    if (bowser.ios && bowser.chrome) {
      ...
      return (
        <ConferencePreConfigContainer constraints={constraints} handleJoin={this.handleJoin} />
      )
