...
class ConferencePreConfigContainer extends Component {
    ...
    restartCamera() {
        ...
        if (this.state.videoDeviceSelected != null) videoConstraints = { deviceId: { exact: this.state.videoDeviceSelected } }
        navigator.mediaDevices.getUserMedia({ audio: { deviceId: { exact: this.state.audioDeviceSelected } }, video: videoConstraints })
        .then((stream) => {
    ...
    setAudioDevice(e) {
        ...
        let videoConstraints = false
        if (this.state.videoDeviceSelected != null && this.state.videoEnabled) videoConstraints = { deviceId: { exact: this.state.videoDeviceSelected } }
        navigator.mediaDevices.getUserMedia({ audio: { exact : { deviceId: deviceId } }, video: videoConstraints })
        .then((stream) => {
            this.props.dispatch(InputManagerActions.inputAudioChange(deviceId))
            this.setState({ audioDeviceSelected: deviceId, userStream: stream, lockJoin: false })
            var date = new Date();
            date.setDate(date.getDate() + 365);
            Cookies.set('input', deviceId, { path: '/', expires: date });
            if (this.state.videoDeviceSelected != null) navigator.attachMediaStream(this.video, stream)
        })
    }
    ...
    setVideoDevice(e) {
        ...
        this.setState({ lockJoin: true })
        const deviceId = e.target.value
        navigator.mediaDevices.getUserMedia({ audio: { exact : { deviceId: this.state.audioDeviceSelected } }, video: { deviceId: { exact: deviceId } } })
        .then((stream) => {
            this.props.dispatch(InputManagerActions.inputVideoChange(deviceId))
            this.setState({ videoDeviceSelected: deviceId, userStream: stream, lockJoin: false })
            var date = new Date();
            date.setDate(date.getDate() + 365);
    ...
    componentDidMount() {
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
            navigator.mediaDevices.getUserMedia({ audio: true, video: this.state.videoEnabled ? true : false })
            .then((stream) => {
                ...
                .then((sources) => {
                    ...
                    if (bowser.chrome && resultAudioOutput) {
                        if (!outputCookieExist) {
                            var date = new Date();
                            date.setDate(date.getDate() + 365);
                            Cookies.set('output', resultAudioOutput[0].deviceId, { path: '/', expires: date });
                            this.props.dispatch(InputManagerActions.outputAudioChange(resultAudioOutput[0].deviceId))
                            this.setState({ outputDevices: resultAudioOutput, outputDeviceSelected: resultAudioOutput[0] })
                        } else {
                            this.props.dispatch(InputManagerActions.outputAudioChange(Cookies.get('output')))
                            this.setState({ outputDevices: resultAudioOutput, outputDeviceSelected: Cookies.get('output') })
                        }
                    }
                    ...
                    if (resultVideo.length > 0) {
                        if (!videoCookieExist) {
                            var date = new Date();
                            date.setDate(date.getDate() + 365);
                            Cookies.set('camera', resultVideo[0].deviceId, { path: '/', expires: date });
                            this.props.dispatch(InputManagerActions.inputVideoChange(resultVideo[0].deviceId))
                            this.setState({ videoDevices: resultVideo, videoDeviceSelected: resultVideo[0].deviceId })
                        } else {
                            this.props.dispatch(InputManagerActions.inputVideoChange(Cookies.get('camera')))
                            this.setState({ videoDevices: resultVideo, videoDeviceSelected: Cookies.get('camera') })
                        }
                    ...
                    if (resultAudio.length > 0) {
                        if (!inputCookieExist) {
                            var date = new Date();
                            date.setDate(date.getDate() + 365);
                            Cookies.set('input', resultAudio[0].deviceId, { path: '/', expires: date });
                            this.props.dispatch(InputManagerActions.inputAudioChange(resultAudio[0].deviceId))
                            this.setState({ audioDevices: resultAudio, audioDeviceSelected: resultAudio[0].deviceId })
                        } else {
                            this.props.dispatch(InputManagerActions.inputAudioChange(Cookies.get('input')))
                            this.setState({ audioDevices: resultAudio, audioDeviceSelected: Cookies.get('input') })
                        }
                    } else {
                        this.setState({ error: strings.noAudioDevice })
                    }

                    /* GETUSERMEDIA FROM PREVIOUS MANAGMENT */
                    if (resultAudio.length > 0) {
                        navigator.mediaDevices.getUserMedia({ audio: true, video: resultVideo.length > 0 && this.state.videoEnabled ? { deviceId: { exact: this.state.videoDeviceSelected } } : false })
                        .then((stream) => {
                            navigator.attachMediaStream(this.video, stream)
    ...
    render() {
        const { handleJoin } = this.props 
        const { level, videoEnabled, audioEnabled, videoDeviceSelected, audioDeviceSelected, outputDeviceSelected, error } = this.state
        return (
                        ...
                        <div className="settings-preconfig-container">
                            <div className="settings-preconfig">
                                <div className="content">
                                    <h3>{error == null ? strings.titlePreConfig : strings.error}</h3>
                                    { error == null ?
                                    <form>
                                            ...
                                            { audioEnabled &&
                                                <div>
                                                    <div className="form-group">
                                                        ...
                                                        <select name="audio" className="form-control" value={audioDeviceSelected != null ? audioDeviceSelected : ""} disabled={false} onChange={this.setAudioDevice}>
                                                    ...
                                                    <div className="form-group">
                                                        <PreConfigVuMeter  stream={this.state.userStream}/>
                                                    </div>
                                                </div>
                                            }
                                            { bowser.chrome &&
                                                <div className="form-group">
                                                    ...
                                                    <select name="output" className="form-control"  value={outputDeviceSelected != null ? outputDeviceSelected : ""} disabled={false} onChange={this.setOutputDevice}>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <select name="video" value={videoDeviceSelected != null ? videoDeviceSelected : ""} className="form-control" disabled={this.state.videoEnabled ? false : true} onChange={this.setVideoDevice}>
                                        ...
                                        <div className="form-group group-enable">
                                            <div>
                                                ...
                                                />
                                                <label htmlFor="videoEnabled">{strings.video}</label>
                                            </div>
                                        </div>
                                        <div>
                                            <button type="button" disabled={this.state.lockJoin} className="start-conference" onClick={this.handleJoin}>
                                                {strings.joincall}
                                            </button>
                                        </div>
                                    </form>
