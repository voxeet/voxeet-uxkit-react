...
export const getOrganizedPosition = ({ width, height, size, index, ...params }) => {
  ...
  const index_t2 = Math.floor(size_t / 2) - ((size_t - 1) % 2) + ((index_t % 2) * 2 - 1) * Math.ceil(index_t / 2);
...
const getPosYmedian = ({ positions, participants }) => {
  const posYsum = participants.reduce((sum, p) => sum + getPosY(positions[p.id]), 0);
  return posYsum / participants.length;
};

const getSameLineParticipants = ({ positions, participants, posYmedian, participantId }) => {
...
const getTileHeight = ({ sameLineParticipants, dimensions, participants }) => {
  ...
  const otherLineParticipantsLength = participants.length - sameLineParticipants.length;
...
const getXOrder = ({ sameLineParticipants, positions, participantId }) => {
  const oderedSameLineParticipants = sameLineParticipants.sort((p1, p2) => (
    positions[p1.id] && positions[p2.id] ? positions[p1.id].posX - positions[p2.id].posX : 0
  ));
  return oderedSameLineParticipants.findIndex(p => p.id === participantId);
};

const getTilePosY = ({ sameLineParticipants, participants, positions, participantId, posYmedian, tH }) => {
...
const getTilePosition = ({ positions, participants, participant, dimensions }) => {
  const participantId = participant.userId;
  const posYmedian = getPosYmedian({ positions, participants });
  const sameLineParticipants = getSameLineParticipants({ positions, participants, posYmedian, participantId });
  ...
  const tY = getTilePosY({ sameLineParticipants, participants, positions, participantId, posYmedian, tH });
  return { tX: xOrder * tW, tY, tW, tH };
};

const getSpeakerModePosition = ({ positions, participants, participant, dimensions }) => {
  const participantId = participant.userId;
...
export const getPosition = ({ mode, positions, participants, participant, dimensions }) => {
  switch (mode) {
  case 'gallery':
    return getTilePosition({ positions, participants, participant, dimensions });
  case 'speaker':
    return getSpeakerModePosition({ positions, participants, participant, dimensions });
  case 'room':
  default:
    const { posX, posY } = positions[participant.userId] || {posX: 0, posY: 0};
